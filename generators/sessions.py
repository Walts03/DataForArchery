...
for id, competition in enumerate(competitions):
	...
	for day in range(competition['Duration']):
		for hour in shoot_hour:
			...
			for archer_id in random.sample(range(1, 4), k=2): 
				...
				arrows = []
				# Insert Arrows
				for dist_shooting_range, sub_round_data in random.sample(sorted(round_data['SubRounds'].items()), k=random.randint(1,2)):
